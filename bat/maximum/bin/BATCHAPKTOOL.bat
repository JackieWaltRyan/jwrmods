...
set adb_mode=off
set signing_scheme=v1
set delete_resources_in_expert_mode=OFF
if not "!workdir!"=="." (
	if exist "!workdir!" (
		cd "!workdir!"
		set bindir=..\bin\
...
if exist batchapktool.ini (
	...
	if "!use_aapt2:~0,1!"=="O" (set use_aapt2=AUTO)
)
...
echo   5  !str_dontwritedebug! : !str_%deldebuginfo%!
echo.
...
FOR %%F IN ("_INPUT_APK\!apk_base!.apk") DO (
	...
	7z x -tzip "_INPUT_APK\%%~nxF" -o"_INPUT_APK\%%~nF\_backup" *.dex -aoa -sccWIN >nul 2>>%logD%
...
FOR /D %%F IN ("_INPUT_APK\!apk_base!") DO (
	...
	if %use_aapt2%==AAPT2 (set opt=--use-aapt2)
	cecho [*] !str_recompiling! %%~nxF...
	echo.>>%logR%
	echo [*] !str_recompiling! %%~nxF !opt2!>>%logR%
...
if !manifest_changed!==1 (
	call :copy2 "_INPUT_APK\%~1\build\apk\AndroidManifest.xml" "_INPUT_APK\%~1\_backup\upd\7\AndroidManifest.xml"
)
call :check_compression "_INPUT_APK\%~1"
FOR %%S IN ("_INPUT_APK\%~1\build\apk\*.dex") DO (
	call :copy2 "_INPUT_APK\%~1\build\apk\%%~nxS" "_INPUT_APK\%~1\_backup\upd\!comp_level!\%%~nxS"
)
...
if exist "_INPUT_APK\%~1\_backup\upd\7" (
	7z a -tzip "_INPUT_APK\%~1\dist\%~1.apk" ".\_INPUT_APK\%~1\_backup\upd\7\*" -mx7 >nul 2>nul
	if errorlevel 1 (
		pushd "_INPUT_APK\%~1\_backup\upd\7"
...
call :check_compression "_INPUT_APK\%~1"
7z a -tzip "_INPUT_APK\%~1\dist\%~1.apk" ".\_INPUT_APK\%~1\_backup\*.dex" -up1q1r2x1y1z1w1 -mx!comp_level! >nul 2>nul
goto:eof
:fix_apktool_yml
set yml_line=0
...
if exist "%~1\apktool.yml" (
	FOR /f "delims=" %%a IN ('yq_windows read "%~1\apktool.yml" doNotCompress 2^>nul') DO (
		if "%%~a"=="- classes.dex" set comp_level=0
		if "%%~a"=="- dex" set comp_level=0
	)
)
goto:eof
:check_manifest
...
7z d -tzip "%bindir%temp\%~n1_%~x1" META-INF\*.MF META-INF\*.SF META-INF\*.RSA META-INF\*.DSA META-INF\*.EC META-INF\sig-* >nul 2>nul
if errorlevel 1 (
	zip -d -ic "%bindir%temp\%~n1_%~x1" META-INF/*.MF META-INF/*.SF META-INF/*.RSA META-INF/*.DSA META-INF/*.EC META-INF/sig-* >nul %~3
)
if /I "%~x1"==".apk" (
	zipalign -f -p 4 "%bindir%temp\%~n1_%~x1" "%~2" >nul %~3
	if "!errorlevel!"=="0" (
		if "!signing_scheme!"=="v1" (
			java -Xmx1024M -Xss1m -jar "%bindir%apksigner.jar" sign --key "%bindir%!key_name!" --cert "%bindir%!key_base!.x509.pem" --min-sdk-version 10 "%~2" >nul %~3
		) else (
			java -Xmx1024M -Xss1m -jar "%bindir%apksigner.jar" sign --key "%bindir%!key_name!" --cert "%bindir%!key_base!.x509.pem" "%~2" >nul %~3
		)
	...
	java -Xmx1024M -Xss1m -jar "%bindir%apksigner.jar" sign --key "%bindir%!key_name!" --cert "%bindir%!key_base!.x509.pem" --out "%~2" --min-sdk-version 10 --v2-signing-enabled false --v3-signing-enabled false "%bindir%temp\%~n1_%~x1" >nul %~3
	if not "!errorlevel!"=="0" set sign_error=1
)
if "!sign_error!"=="1" (
	set /A error_count+=1
...
del /f /q "%bindir%temp\%~n1_%~x1" 2>nul
goto:eof
:zipalignoutapks
set error_count=0
call :measure_time start
...
:javasource
set temp_name=
for /f "delims=" %%a in ('FileToOpen "set temp_name=" "!cd!\*.jadx;*.apk;*.dex;*.jar;*.class;*.smali;*.zip;*.aar;*.arsc" "!str_dragajd!"') do %%a
if not '!temp_name!'=='' (
...
if '%~2'=='apktool' (
	if %use_aapt2%==AUTO (
		...
		echo APKTOOL (%use_aapt2%^)              : !apktool!>>%~1
	)
...
if '%~2'=='deodex' (
	echo DEODEXER                     : !smali!>>%~1
	echo !str_apilevel_log! : !api!>>%~1
)
if '%~2'=='deodex_oat2dex' (
	echo DEODEXER                     : oat2dex>>%~1
	echo !str_apilevel_log! : !api!>>%~1
)
if '%~2'=='deodex_vdex' (
	if '%api%'=='28' (
		echo DEODEXER                     : vdexExtractor (!cdex_converter!^)>>%~1
	) else (
		echo DEODEXER                     : vdexExtractor>>%~1
	)
	echo !str_apilevel_log! : !api!>>%~1
)
echo -------------------------------------------------->>%~1
...
echo !bat_line!
echo   30              Android 11.0 (R)
echo   29              Android 10.0 (Q)
...
goto restart
:setaapt
if %use_aapt2%==AUTO (set use_aapt2=AAPT1& goto setaapt_done)
if %use_aapt2%==AAPT1 (set use_aapt2=AAPT2& goto setaapt_done)
if %use_aapt2%==AAPT2 (set use_aapt2=AUTO)
:setaapt_done
...
7z a -tzip "%bindir%temp\%~nx1" ".\%~2" -mx0 >nul 2>nul
if errorlevel 1 (
	zip -j -0 "%bindir%temp\%~nx1" "%~2" >nul 2>>%logX%
)
...
if %api% GEQ 28 goto deodex_vdexExtractor
if %api% GEQ 26 (
...
rd /s /q "%~2\%~n1" 2>nul
goto:eof
:deodex_vdexExtractor
if %vOS% LSS 7600 (
	set cdex_converter=adb
	if %adb_mode%==off (
		cecho {!colR!}[*] !str_device_warning!{# #}{\n}
		pause
		goto restart
	)
	::FOR /f "delims=" %%a IN ('adb shell getprop ro.product.cpu.abi') DO (set device_arch=%%~a)
	adb push "%bindir%compact_dex_converter_arm" "/data/local/tmp/" >nul 2>nul
	if not "!errorlevel!"=="0" (
		cecho {!colR!}[*] !str_error! Device is read-only^^!{# #}{\n}
		pause
		goto restart
	)
	adb shell chmod 755 '/data/local/tmp/compact_dex_converter_arm' >nul 2>nul
) else (
	set cdex_converter=flinux
	if %vOS% GEQ 14393 (
		if "%xOS%"=="x64" (
			wslconfig /list >nul 2>nul
			if "!errorlevel!"=="0" (
				set cdex_converter=wsl
				if "%bindir%"=="bin\" (set wsl_bindir=./bin/) else (set wsl_bindir=../bin/)
			)
		)
	)
)
call :log_version "%logX%" "deodex_vdex"
cecho [*] !str_selectedarch! {!colG!}!FARCH1!{# #}{\n}
echo [*] !str_selectedarch! !FARCH1!>>%logX%
set boot_oat_file=
java -Xmx1024m -jar "%bindir%bak!smali!" l d "_system\framework\!FARCH1!\boot.oat" >"_system\framework\!FARCH1!\boot.list" 2>>%logX%
set /P boot_oat_file=<"_system\framework\!FARCH1!\boot.list"
del /f /q "_system\framework\!FARCH1!\boot.list" 2>nul
if not "!boot_oat_file!"=="" (
	FOR /f "delims=" %%a IN ("!boot_oat_file!") DO (set "boot_oat_file=%%~nxa")
	set _temp_=0
	FOR %%a IN ("flyme-framework.jar" "qtiNetworkLib.jar" "com.qualcomm.qti.camera.jar" "com.nxp.nfc.nq.jar" "core-oj.jar") DO (
		if /I "!boot_oat_file!"=="%%~a" set _temp_=1
	)
	if !_temp_!==0 echo boot.oat=!boot_oat_file! >>%logX%
) else (
	FOR %%a IN ("flyme-framework.jar" "qtiNetworkLib.jar" "com.qualcomm.qti.camera.jar" "com.nxp.nfc.nq.jar" "core-oj.jar") DO (
		if exist "_system\framework\%%~a" (
			if not exist "_system\framework\!FARCH1!\boot-%%~na.oat" set "boot_oat_file=%%~a"
		)
	)
	if "!boot_oat_file!"=="" echo boot.oat=? >>%logX%
)
set count2=0
set count3=0
FOR /f "delims=" %%a IN ('sfk list -quiet -quot -dir _system -minsize^=22b -file .jar .apk 2^>nul') DO (
	set file_deodexed=0
	call :GetParent "%%~a" "parent"
	if "%%~nxa"=="!boot_oat_file!" (set boot_file=boot.vdex) else (set boot_file=boot-%%~na.vdex)
	if exist "!parent!\%%~na.vdex" (
		set file_deodexed=1
		set /A count2+=1
	)
	FOR /L %%b IN (1,1,!count_arch!) DO (
		if !file_deodexed!==0 (
			if exist "!parent!\oat\!FARCH%%b!\%%~na.vdex" (
				set file_deodexed=1
				set /A count2+=1
			)
		)
	...
	if !file_deodexed!==0 (
		if exist "_system\framework\!boot_file!" (
			set file_deodexed=1
			set /A count2+=1
		)
	)
	FOR /L %%b IN (1,1,!count_arch!) DO (
		if !file_deodexed!==0 (
			if exist "_system\framework\!FARCH%%b!\!boot_file!" (
				set file_deodexed=1
				set /A count2+=1
			)
		)
...
FOR /f "delims=" %%a IN ('sfk list -quiet -quot -dir _system -minsize^=22b -file .jar +then list -quiet -quot -dir _system -minsize^=22b -file .apk 2^>nul') DO (
	...
	if exist "!parent!\%%~na.vdex" (
		set /A count3+=1
		title !title_text! [!count3!/!count2!]
		call :deodex_4 "!parent!\%%~na.vdex" "!parent!" "%%~a" classes.dex
	)
	FOR /L %%b IN (1,1,!count_arch!) DO (
		if !file_deodexed!==0 (
			if exist "!parent!\oat\!FARCH%%b!\%%~na.vdex" (
				set /A count3+=1
				title !title_text! [!count3!/!count2!]
				call :deodex_4 "!parent!\oat\!FARCH%%b!\%%~na.vdex" "!parent!\oat\!FARCH%%b!" "%%~a" classes.dex
			)
		)
	)
	if !file_deodexed!==0 (
		if exist "_system\framework\!boot_file!" (
			set /A count3+=1
			title !title_text! [!count3!/!count2!]
			call :deodex_4 "_system\framework\!boot_file!" "_system\framework" "%%~a" classes.dex
		)
	)
	FOR /L %%b IN (1,1,!count_arch!) DO (
		if !file_deodexed!==0 (
			if exist "_system\framework\!FARCH%%b!\!boot_file!" (
				set /A count3+=1
				title !title_text! [!count3!/!count2!]
				call :deodex_4 "_system\framework\!FARCH%%b!\!boot_file!" "_system\framework\!FARCH%%b!" "%%~a" classes.dex
			)
		)
	)
	if !file_deodexed!==1 (
		del /f /q "!parent!\%%~na.vdex" 2>nul
		FOR /L %%b IN (1,1,!count_arch!) DO (
			del /f /q "!parent!\oat\!FARCH%%b!\%%~na.*dex*" "!parent!\oat\!FARCH%%b!\%%~na.art" 2>nul
			rd "!parent!\oat\!FARCH%%b!" 2>nul
		)
		rd "!parent!\oat" 2>nul
	)
)
if "!cdex_converter!"=="adb" (
	adb shell rm '/data/local/tmp/compact_dex_converter_arm' >nul 2>nul
)
call :symlinks_search
call :done "%logX%"
goto restart
...
if exist "%~2\%~n1_classes.cdex" (
	set cdex_count=0
	FOR %%S IN ("%~2\%~n1_classes*.cdex") DO (
		set /A cdex_count+=1
		set REN1=%%~nS
		set REN1=!REN1:%~n1_=!
		if "!cdex_converter!"=="adb" (
			adb push "%%~S" "/data/local/tmp/" >nul 2>nul
			adb shell '/data/local/tmp/compact_dex_converter_arm' '/data/local/tmp/%%~nxS' >>%logX% 2>&1
			adb pull "/data/local/tmp/%%~nxS.new" "%~2\!REN1!.dex" >nul 2>nul
			adb shell rm '/data/local/tmp/%%~nxS' '/data/local/tmp/%%~nxS.new' >nul 2>nul
		)
		if "!cdex_converter!"=="flinux" (
			if "!workdir!"=="." (
				ConH /Wait flinux "bin\compact_dex_converter_linux_32" "%%~S" >>%logX% 2>&1
			) else (
				pushd "!rootdir!"
				ConH /Wait flinux "bin\compact_dex_converter_linux_32" "!workdir!\%%~S" >>"!workdir!\%logX%" 2>&1
				popd
			)
			ren "%%~S.new" "!REN1!.dex" >nul 2>nul
		)
		if "!cdex_converter!"=="wsl" (
			set REN2=%~2
			set REN2=!REN2:\=/!
			wsl '%wsl_bindir%compact_dex_converter_linux_64' './!REN2!/%%~nxS' 2>&1 | nhrt -set:"\r" -et:"\r\n" -notitle >>%logX% 2>&1
			ren "%~2\%%~nxS.new" "!REN1!.dex" >nul 2>nul
		)
	)
	FOR %%S IN ("%~2\classes*.dex") DO (set /A cdex_count-=1)
	if "!cdex_count!"=="0" (set file_deodexed=1) else (set file_deodexed=2)
)
...
FOR %%F IN ("_system\odex.*.sqsh") DO (
	set x=%%~nxF
	7z x "%%~F" -o"_system\!x:~5,-5!\" -aou -sccWIN >nul 2>>%logX%
	if errorlevel 1 (
...
FOR %%F IN ("_system\odex*.sqsh") DO (
	7z x "%%~F" -o"_system\" -aou -sccWIN >nul 2>>%logX%
	if errorlevel 1 (
		cecho {!colR!}[*] !str_errorunp! %%~nxF{# #}{\n}
		echo [*] ---^> !str_errorunp! %%~nxF>>%logX%
	) else (
...
FOR /f "delims=" %%a IN ('sfk list -quiet -quot -dir _system -file .odex.xz .odex.gz 2^>nul') DO (
	call :GetParent "%%~a" "parent"
	7z x "%%a" -o"!parent!" -aoa -sccWIN >nul 2>>%logX%
	if errorlevel 1 (
...
:baksmali
rd /s /q "%~2\%~n1" 2>nul
7z x -tzip "%~2\%~nx1" -o"%~2\%~n1" *.dex -aoa -sccWIN >nul 2>>%logD%
set count2=0
...
7z a -tzip "%bindir%temp\%~nx1" ".\%~2\%~n1\*.dex" -mx0 >nul 2>nul
if errorlevel 1 (
	zip -j -0 "%bindir%temp\%~nx1" "%~2\%~n1\*.dex" >nul 2>>%logR%
)
if /I "%~2"=="_INPUT_JAR" (
	zipalign -f -p 4 "%bindir%temp\%~nx1" "_OUT_JAR\%~nx1" >nul 2>>%logR%
)
if /I "%~2"=="_INPUT_APK" (
	if %sign_after%==ON (
